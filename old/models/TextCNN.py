...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.embedding_pretrained is not None:
            # nn.Embedding  arg0:dict_size  arg1:embed_dim
    ...
    def conv_and_pool(self, x, conv):
        #x: [B, 1, L, emb] ->(conv)-> [B, num_filters, L-k+1, 1] ->(squeeze)-> [B, num_filters, L-k+1]
        x = F.relu(conv(x)).squeeze(3)
        #x: [B, num_filters, L-k+1] ->(max_pool1d)-> [B, num_filters, 1] ->(squeeze)-> [B, num_filters]
